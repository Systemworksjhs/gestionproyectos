<?php
    ...
    class Consulta{
        ...
        public function buscar(){
            ...
            if($tipo_usuario == 1){
                $where = "WHERE str_to_date(left(date_created_order,10), '%Y-%m-%d')>=date('$inicio') AND str_to_date(left(date_created_order,10), '%Y-%m-%d')<=date('$fin') ORDER BY id_order DESC";
            }
            if($tipo_usuario == 6 or $tipo_usuario == 5){
                $where = "WHERE str_to_date(left(date_created_order,10), '%Y-%m-%d')>=date('$inicio') AND str_to_date(left(date_created_order,10), '%Y-%m-%d')<=date('$fin') and id_municipality=$ids AND (status='Aprobado' OR status='Alistamiento' OR status='En proceso' OR status='Empacado' OR status='En ruta' OR status='Listo para recoger' OR status='Entregado') ORDER BY id_order DESC";
            }	
    ...
    if($_SESSION['kind'] == 5 or ($_SESSION['kind'] == 6 and $_SESSION["user_id"]!=29) or $_SESSION["user_id"]==40){
    ...
    //Plantilla para Deyvi todos los municipios
    if($_SESSION["user_id"]==29){
        //echo "id usuario :". $_SESSION["user_id"];
        $usuario = new Consulta();
        $salida ="";
        $salida .= "<br>";
        $salida .= "<hr>";
