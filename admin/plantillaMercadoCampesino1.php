<?php
    ...
    class Consulta{
        ...
        public function buscar(){
            ...
            //echo "Municipio   : $ids <br>";
                $where = "WHERE str_to_date(left(date_created_order,10), '%Y-%m-%d')>=date('$inicio') AND str_to_date(left(date_created_order,10), '%Y-%m-%d')<=date('$fin') and (funcionario=1 OR funcionario=0 ) AND (details LIKE '%Mercado%') ORDER BY id_order DESC";
    ...
    $salida .= "<thead>
                ...
                foreach($usuario->buscar() as $r){
                    ...
                    {
                        if($r->name_user == ""){
                            $nameU=utf8_decode($results['names']).' '.utf8_decode($results['lastnames']);
                        }else{
                            $nameU=$r->name_user;
                        }
                        
                        if($r->identification_user == ""){
                            $ident=utf8_decode($results['identificacion']);
                        }else{
                            $ident=$r->identification_user;
                        }
                        $salida .= 
                    ...
                    "<tr style='BACKGROUND-COLOR: #F2F2F2'> 
                        ...
                        <td style='BACKGROUND-COLOR: #F2F2F2'>".utf8_decode($r->address)."</td>
