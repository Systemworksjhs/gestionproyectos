<?php
	...
	// partiendo de que ya tenemos creado un objecto PostData previamente utilizamos el contexto
		public function update(){
			$sql = "update ".self::$tablename." set title=\"$this->title\",brief=\"$this->brief\",content=\"$this->content\",image=\"$this->image\",urlpost=\"$this->urlpost\",registrationForm=\"$this->registrationForm\",category_id=\"$this->category_id\",status=$this->status where id=$this->id";
			Executor::doit($sql);
		}
		...
		public static function getAllActiveMain(){
			$sql = "select * from ".self::$tablename." where status=1 order by id desc LIMIT 3";
			$query = Executor::doit($sql);
			return Model::many($query[0],new PostData());
