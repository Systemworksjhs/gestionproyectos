<?php
	class productosDptoNarinoData {
		public static $tablename = "productos_dpto_narino";

		public function productosDptoNarinoData(){
			$this->name = "";
		}

		public function add(){
			$fecha_Creacion =date("d-m-Y h:i:s");
			$sql = "insert into productos_dpto_narino (nombre_Producto, imagen, price , unit_of_measurement, estado, clase, date_Creation) ";
		...
		// partiendo de que ya tenemos creado un objecto productosDptoNarinoData previamente utilizamos el contexto
		public function update(){
			$sql = "update ".self::$tablename." set nombre_Producto=\"$this->nombre_Producto\",estado=\"$this->estado\",imagen=\"$this->imagen\",price=\"$this->price\",unit_of_measurement=\"$this->unit_of_measurement\",clase=\"$this->clase\",date_update=NOW() where id_product=$this->id_product";
			Executor::doit($sql);
		}
		...
		public static function getById($id){
			...
			return Model::one($query[0],new productosDptoNarinoData());
		}

		public static function getAll(){
			$sql = "select * from ".self::$tablename;
			$query = Executor::doit($sql);
			return Model::many($query[0],new productosDptoNarinoData());
		}
		...
		public static function getAllRecents(){
			$sql = "SELECT * FROM ".self::$tablename." WHERE estado=1 order by id_product desc LIMIT 5";
			$query = Executor::doit($sql);
			return Model::many($query[0],new productosDptoNarinoData());
