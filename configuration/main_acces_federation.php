<?php
    ...
    if (!empty($_POST)) {
        ...
        $emailFederation = $mysqli->real_escape_string($_POST['emailfederation']);
        ...
        if (count($errors) == 0){
            ...
            //if($arr['success']){
                $registro = registraMessageFederation($sender, $email, $phone, $affair, $message, $fecha, $codemessage, $status);
                if ($registro > 0){
    				...
    				$mail1->addBCC($emailFederation);                 // Add Copy hidden
