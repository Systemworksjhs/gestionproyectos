<?php
    ...
    if(isset($_POST['submitoperador'])){
        $conjunto = $mysqli->real_escape_string($_POST['conjunto']);
        $iniciativa = $mysqli->real_escape_string($_POST['iniciativa']);
        $nit = $mysqli->real_escape_string($_POST['identificacion']);
        $phone = $mysqli->real_escape_string($_POST['phone']);
        $email = $mysqli->real_escape_string($_POST['email']);
        $address = $mysqli->real_escape_string($_POST['ciudad']);        
        $names = $mysqli->real_escape_string($_POST['names']);
        $cargo = $mysqli->real_escape_string($_POST['cargo']);
        $identificacion = $mysqli->real_escape_string($_POST['nit']);
        $genero =  $mysqli->real_escape_string($_POST['genero']);
        $id_municipality_user = $mysqli->real_escape_string($_POST['id_municipality_user']);
        $operador = $mysqli->real_escape_string($_POST['operador']);
        $usuario = $mysqli->real_escape_string($_POST['nit']);
        $password = $mysqli->real_escape_string($_POST['password']);
        $con_password = $mysqli->real_escape_string($_POST['con_password']);
        $id_tipo          = 3;
        $rol              = "Compras";
        ...
        //    $errors[] = "Por favor verifica el captcha";
        //}
        if (isNullThree($usuario, $names, $nit, $identificacion, $password, $con_password, $email, $id_municipality_user, $phone, $id_tipo, $rol, $activacion, $address)){
            $errors[] = "Debe digitar todos los campos...";
        }
        ...
        if(nitExiste($nit)){
            $errors[] = "El número nit $nit ya está registrado";
        }
        ...
        if (count($errors) == 0){
            ...
            //$arr = json_decode($response, TRUE);
            //if($arr['success']){
                $registro = registraUserCompraPublicas($usuario,$pass_hash,$names,$genero,$email,$activacion,$token,$token_password,$password_request,$id_tipo,$identificacion,$phone,$rol,$id_municipality_user,$address,$imagen,$conjunto,$iniciativa,$nit,$cargo,$operador,$fecha);
                if ($registro > 0){
                    //$url = 'http://'.$_SERVER["SERVER_NAME"].'/login/activar.php?id='.$registro.'&val='.$token;
                    //$asunto = "Activar cuenta en el la plataforma de AgrotiNariño";
                    //$cuerpo = "Estimado .$names. $lastnames ; <br /><br />para continuar con el proceso de registro, es indispensable de click en el siguiente link <a href='$url'>Activar cuenta</a>";
                    ...
                    //    $rout = "admin/uploads/profiles/".$imagenAux;
                    //    $outcome = @move_uploaded_file($_FILES["image"]["tmp_name"], $rout);
                        $_SESSION['message'] = "Actividad registrada en la base de datos, pronto será activado en el sistema.";
                    //}
        			//Envio de correo electrónico
    				$mail1 = new PHPMailer;
    				//$mail->SMTPDebug = 3;
